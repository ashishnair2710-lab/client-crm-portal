...
const SERVICES  = CLIENT_CONFIG.agent.services
const LOCATIONS = CLIENT_CONFIG.agent.locations

function buildSystemPrompt(form: FormData): string {
  return CLIENT_CONFIG.agent.systemPrompt(form.name, form.interest, form.location)
}
    ...
    try {
      ...
      const callConfig: any = form.assistantId.trim()
        ...
        : {
            ...
            firstMessage: `Hello ${form.name || 'there'}! Thanks for reaching out to ${CLIENT_CONFIG.name}. I see you're interested in our ${form.interest.split('(')[0].trim()}. ${CLIENT_CONFIG.agent.greeting}`,
